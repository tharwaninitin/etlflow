...
object SparkReadWriteStep {
  def apply[T <: Product: TypeTag, O <: Product: TypeTag](
             ...
             ,output_save_mode: SaveMode = SaveMode.Append
             ,output_repartitioning: Boolean = false
             ,transform_function: Option[InputDataset[T] => OutputDataset[O]] = None
           ) (spark: => SparkSession): SparkReadWriteStateStep[T,Unit,O,Unit] = {
    new SparkReadWriteStateStep[T,Unit,O,Unit](name,input_location,input_type,output_location,output_type,output_filename,output_partition_col,output_save_mode,output_repartitioning,transform_function)(spark)
  }
}