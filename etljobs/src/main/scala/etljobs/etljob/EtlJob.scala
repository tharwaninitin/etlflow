...
import etljobs.{EtlJobProps, LoggerResource}
import etljobs.bigquery.BigQueryManager
import etljobs.jdbc.DbManager
...
trait EtlJob extends BootstrapRuntime with SparkManager with BigQueryManager with DbManager {
  ...
  private[etljob] val managed_etl_resource: ZManaged[Any, Throwable, LoggerResource] = for {
  ...
  private[etljob] def logJobInit(res: LoggerResource): ZIO[Any, Throwable, Long] =
    if (res.db.isDefined) res.db.get.updateJobInformation("started","insert") else ZIO.succeed(0)

  private[etljob] def logJobError(e: Throwable, job_start_time: Long)(res: LoggerResource): Throwable = {
  ...
  private[etljob] def logJobSuccess(job_start_time: Long)(res: LoggerResource): ZIO[Any, Throwable, Unit] = {
    for {
