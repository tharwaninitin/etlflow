...
class ReadWriteSparkStep(
                val name : String
                ,val etl_metadata: Map[String, String] = Map[String, String]()
                ,source_dataset: => Dataset[Row]
                ,transform_function : Option[Dataset[Row] => Dataset[Row]] = None
...
extends EtlStep[Unit,Unit]
{
  ...
  def showCorruptedData(): Unit = {
    ...
    dataset.filter("_corrupt_record is not null").show(10)
  }
}
