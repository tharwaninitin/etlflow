...
private[etlflow] object S3Impl {
  val live: ZLayer[S3AsyncClient, Throwable, S3Api] = ZLayer.fromFunction{s3: S3AsyncClient =>
    new Service {
      ...
      def listBucketObjects(bucket: String, prefix: String, maxKeys: Int = 20): Task[ListObjectsV2Response] = Task.effectAsync[ListObjectsV2Response] { callback =>
        processResponse(
          s3.listObjectsV2(
      ...
      def putObject(bucket: String, key: String, file: String): Task[PutObjectResponse] = Task.effectAsync[PutObjectResponse] { callback =>
        processResponse(
      ...
      def getObject(bucket: String, key: String, file: String): Task[GetObjectResponse] = Task.effectAsync[GetObjectResponse] { callback =>
        processResponse(
      ...
      def delObject(bucket: String, key: String): Task[DeleteObjectResponse] = Task.effectAsync[DeleteObjectResponse] { callback =>
        processResponse(
