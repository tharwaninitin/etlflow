...
trait GenericEtlJob[EJP <: EtlJobProps] extends EtlJob[EJP] {
  ...
  final def execute(job_run_id: Option[String] = None, is_master: Option[String] = None): ZIO[ZEnv, Throwable, Unit] = {
    (for {
      ...
      slack           = SlackLogger(job_name, job_properties, config.slack.map(_.env).getOrElse(""), config.slack.map(_.url).getOrElse(""))
      db              <- DbLogger(job_name, job_properties, config, Platform.default.executor.asEC, blocker, job_name + "-Pool", jri, master_job)
