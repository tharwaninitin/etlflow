...
case class ParallelETLStep(name: String)(steps: EtlStep[Unit,Unit]*) extends EtlStep[Unit,Unit] {
  ...
  final def process(in: => Unit): RIO[CoreEnv, Unit] = {
    ...
    (for{
      ...
      _   <- ZIO.foreachPar(steps)(x => x.execute())
    } yield ()).provideSomeLayer[DBEnv with JsonEnv with CryptoEnv  with  Blocking with Clock](etlflow.log.Implementation.live())
  }
