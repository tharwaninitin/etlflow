...
object DP {
  ...
  private def submitAndWaitForJobCompletion(jobControllerClient: JobControllerClient, projectId: String,
    ...
    gcp_logger.info(s"Submitted job $jobId")
    var continue = true
    ...
    while (continue) {
      ...
      gcp_logger.info(s"Job Status $jobState")
      jobInfo.getStatus.getState.toString match {
        case "DONE" =>
          gcp_logger.info(s"Job $jobId completed successfully with state $jobState")
          continue = false
        case "CANCELLED" | "ERROR" =>
          gcp_logger.error(s"Job $jobId failed with error ${jobInfo.getStatus.getDetails}")
    ...
    Task {
      ...
      new DPService.Service {
        override def executeSparkJob(
          ...
          gcp_logger.info(s"""DataProc Configurations:
