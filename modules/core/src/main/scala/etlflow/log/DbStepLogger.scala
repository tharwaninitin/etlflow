...
object DbStepLogger extends ApplicationLogger {
  ...
  class StepHelper(etlStep: EtlStep[_,_], job_run_id: String, job_notification_level: LoggingLevel = LoggingLevel.INFO) extends ApplicationLogger {
    ...
    def updateStepLevelInformation(start_time: Long, state_status: String, error_message: Option[String] = None, mode: String = "update"): ZIO[DBServerEnv, Throwable, Unit] =
    {
  ...
  case class StepReq(job_run_id: String, job_notification_level: LoggingLevel = LoggingLevel.INFO)

  type LoggingSupport = Has[LoggerService]
  trait LoggerService {
    def logInit(start_time: Long): ZIO[DBServerEnv, Throwable, Unit]
    def logSuccess(start_time: Long): ZIO[DBServerEnv, Throwable, Unit]
    def logError(start_time: Long, ex: Throwable): ZIO[DBServerEnv, Throwable, Unit]
  }
  def logInit(start_time: Long): ZIO[LoggingSupport with DBServerEnv, Throwable, Unit] = ZIO.accessM(_.get.logInit(start_time))
  def logSuccess(start_time: Long): ZIO[LoggingSupport with DBServerEnv, Throwable, Unit] = ZIO.accessM(_.get.logSuccess(start_time))
  def logError(start_time: Long, ex: Throwable): ZIO[LoggingSupport with DBServerEnv, Throwable, Unit] = ZIO.accessM(_.get.logError(start_time,ex))
