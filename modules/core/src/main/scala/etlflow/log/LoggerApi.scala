...
object LoggerApi {
  ...
  trait Service {
    ...
    def stepLogStart(start_time: Long, etlStep: EtlStep[_,_]): RIO[DBLogEnv with ConsoleEnv, Unit]
    def stepLogEnd(start_time: Long, etlStep: EtlStep[_,_], ex: Option[Throwable]): RIO[DBLogEnv with ConsoleEnv with SlackEnv, Unit]
  }
  ...
  def stepLogStart(start_time: Long, etlStep: EtlStep[_,_]): RIO[LoggerEnv with DBLogEnv with ConsoleEnv, Unit] =
    ZIO.accessM[LoggerEnv with DBLogEnv with ConsoleEnv](_.get.stepLogStart(start_time, etlStep))
  def stepLogEnd(start_time: Long, etlStep: EtlStep[_,_], ex: Option[Throwable] = None): RIO[LoggerEnv with DBLogEnv with ConsoleEnv with SlackEnv, Unit] =
    ZIO.accessM[LoggerEnv with DBLogEnv with ConsoleEnv with SlackEnv](_.get.stepLogEnd(start_time, etlStep, ex))
}
