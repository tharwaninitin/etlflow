...
object SlackApi extends ApplicationLogger {
  ...
  def logStepEnd(start_time: Long, job_notification_level: LoggingLevel, etlstep: EtlStep[_, _], error_message: Option[String] = None): RIO[SlackEnv, Unit] =
    ZIO.accessM[SlackEnv](_.get.logStepEnd(start_time, job_notification_level, etlstep, error_message))
  def logJobEnd(job_name: String, job_run_id: String, job_notification_level: LoggingLevel, start_time: Long, error_message: Option[String] = None): RIO[SlackEnv, Unit] =
    ZIO.accessM[SlackEnv](_.get.logJobEnd(job_name, job_run_id, job_notification_level, start_time, error_message))
  def getSlackNotification: URIO[SlackEnv, String] = ZIO.accessM[SlackEnv](_.get.getSlackNotification)
  ...
  def live(slack: Option[Slack]): ULayer[SlackEnv] = ZLayer.succeed (
    new Service {
