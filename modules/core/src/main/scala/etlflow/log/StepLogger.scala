...
private[etlflow] object StepLogger extends ApplicationLogger {
  ...
  class StepLoggerHelper(etlStep: EtlStep[_,_], job_run_id: String, job_notification_level: LoggingLevel = LoggingLevel.INFO) extends ApplicationLogger {
    ...
    def update(start_time: Long, state_status: String, error_message: Option[String] = None, mode: String = "update"): ZIO[DBEnv with JsonEnv, Throwable, Unit] =
    {
      ...
      if (mode == "insert") {
        ...
        for{
          properties <- JsonApi.convertToJson(etlStep.getStepProperties(job_notification_level))
  ...
  trait LoggerService {
    def logInit(start_time: Long): ZIO[DBEnv with JsonEnv, Throwable, Unit]
    def logSuccess(start_time: Long): ZIO[DBEnv with JsonEnv, Throwable, Unit]
    def logError(start_time: Long, ex: Throwable): ZIO[DBEnv with JsonEnv, Throwable, Unit]
  }
  def logInit(start_time: Long): ZIO[LoggingSupport with DBEnv with JsonEnv, Throwable, Unit] = ZIO.accessM(_.get.logInit(start_time))
  def logSuccess(start_time: Long): ZIO[LoggingSupport with DBEnv with JsonEnv, Throwable, Unit] = ZIO.accessM(_.get.logSuccess(start_time))
  def logError(start_time: Long, ex: Throwable): ZIO[LoggingSupport with DBEnv with JsonEnv, Throwable, Unit] = ZIO.accessM(_.get.logError(start_time,ex))
