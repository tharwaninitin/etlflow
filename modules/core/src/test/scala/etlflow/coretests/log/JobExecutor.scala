...
object JobExecutor {
  def slack(job_name: String, slack_env: String, slack_url: String, job_props: EtlJobProps, job: ZIO[Has[LoggerResource] with ZEnv, Throwable, Unit]): ZIO[ZEnv, Throwable, SlackLogManager] =
    for {
  ...
  def db(job_name: String, job_type: String, job_props: EtlJobProps, job: ZIO[Has[LoggerResource] with ZEnv, Throwable, Unit]): ZIO[ZEnv, Throwable, Unit] =
    for {
      job_start_time  <- UIO.succeed(UF.getCurrentTimestamp)
      job_layer       = ZLayer.succeed(JobLoggerEnv.live(JobLogger(None,None),job_type))
      step_layer      = ZLayer.succeed(LoggerResource(None,None))
