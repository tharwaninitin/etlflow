  ...
  def spec: ZSpec[environment.TestEnvironment, Any] =
    (suite("EtlFlow Slack Log Suite") (
      ...
      testM("Execute job with log level INFO - Failure Case") {
        ...
        val slack_message = unsafeRun(slackInfoLevelExecutor.provideCustomLayer(fullLayer))

        assertM(slackInfoLevelExecutor.foldM(ex => ZIO.succeed(ex.getMessage), _ => ZIO.succeed(slack_message)))(equalTo(message))
      },
      testM("Execute job with log level JOB - Failure Case") {
        ...
        val slack_message = unsafeRun(slackJobLevelExecutor.provideCustomLayer(fullLayer))
        assertM(slackJobLevelExecutor.foldM(ex => ZIO.succeed(ex.getMessage), _ => ZIO.succeed(slack_message)))(equalTo(message))
      },
