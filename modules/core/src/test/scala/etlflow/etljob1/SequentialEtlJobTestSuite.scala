package etlflow.etljob1

import org.scalatest.{FlatSpec, Matchers}
import org.apache.spark.sql.{Dataset, Row}
import etlflow.spark.{ReadApi, SparkManager}
import etlflow.utils.{AppLogger, CSV, GlobalProperties, ORC, PARQUET}
import etlflow.bigquery.{BigQueryManager, QueryApi}
import examples.MyGlobalProperties
import examples.schema.MyEtlJobName.EtlJob1PARQUETtoORCtoBQLocalWith2Steps
...
class SequentialEtlJobTestSuite extends FlatSpec with Matchers {
  ...
  val global_props: MyGlobalProperties = new MyGlobalProperties(canonical_path + "/etljobs/src/test/resources/loaddata.properties") {}
  ...
  // STEP 2: Execute JOB
  val etljob = EtlJobDefinition(EtlJob1PARQUETtoORCtoBQLocalWith2Steps.toString,job_properties=job_props, global_properties=Some(global_props))
  etljob.execute
