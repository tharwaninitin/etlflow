...
private[etlflow] object Sql {
  ...
  def updateJobRun(jobRunId: String, status: String, props: String, elapsedTime: String): SQL[Nothing, NoExtractor] =
    sql""" UPDATE jobrun
              SET status = $status,
                  properties = CAST($props as JSON),
                  elapsed_time = $elapsedTime
  ...
  def insertJobRun(jobRunId: String, name: String, args: String, props: String, startTime: Long): SQL[Nothing, NoExtractor] =
    sql"""INSERT INTO jobrun(
         ...
         VALUES ($jobRunId, $name, CAST($args as JSON), CAST($props as JSON), 'started', '...', $startTime)"""
