...
package object db {
  ...
  object GetCredential extends SQLSyntaxSupport[GetCredential] {
    def apply(rs: WrappedResultSet): GetCredential =  GetCredential(
      rs.string("name"), rs.string("type"), rs.string("valid_from"))
  }

  case class JobLogs(job_name: String,  success: String, failed: String)
  object JobLogs extends SQLSyntaxSupport[JobLogs] {
    def apply(rs: WrappedResultSet): JobLogs =  JobLogs(
      rs.string("job_name"), rs.string("success"), rs.string("failed"))
  }
  ...
  object UserDB extends SQLSyntaxSupport[UserDB] {
    def apply(rs: WrappedResultSet): UserDB = UserDB(
      rs.string("user_name"), rs.string("password"), rs.string("user_active"), rs.string("user_role"))
  }

  case class JobDB(job_name: String, schedule: String, is_active: Boolean)
  object JobDB extends SQLSyntaxSupport[JobDB] {
    def apply(rs: WrappedResultSet): JobDB =  JobDB(
  ...
  object JobDBAll extends SQLSyntaxSupport[JobDBAll] {
    ...
    def apply(rs: WrappedResultSet): JobDBAll =  JobDBAll(
      rs.string("job_name"), rs.string("job_description"), rs.string("schedule"),
      rs.long("failed"),rs.long("success"), rs.boolean("is_active"),
  ...
  private[etlflow] def liveFullDB(db: JDBC, pool_name: String = "EtlFlow-Pool", pool_size: Int = 2): ZLayer[Blocking, Throwable, DBEnv with DBServerEnv] =
    CP.layer(db, pool_name, pool_size) >>> (DBImpl.live ++ DBServerImpl.live)

  private[etlflow] def liveFullDBWithLog(db: JDBC, job_run_id: String, pool_name: String = "EtlFlow-Pool", pool_size: Int = 2): ZLayer[Blocking, Throwable, DBEnv with LogEnv with DBServerEnv] =
    CP.layer(db, pool_name, pool_size) >>> (DBImpl.live ++ log.DB.live(job_run_id) ++ DBServerImpl.live)
}
