...
class DBReadStep[T] private (val name: String, query: => String)(fn: WrappedResultSet => T) extends EtlStep[List[T]] {
...
object DBReadStep {
  def apply[T](name: String, query: => String)(fn: WrappedResultSet => T): DBReadStep[T] = new DBReadStep[T](name, query)(fn)
}
