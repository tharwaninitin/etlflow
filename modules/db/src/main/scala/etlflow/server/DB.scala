  ...
  val live: ZLayer[Has[String], Throwable, DBServerEnv] = ZLayer.fromService { pool_name =>
    new DBServerApi.Service {
      ...
      override def getJobs: IO[DBException, List[JobDBAll]] =
        Task(NamedDB(pool_name).readOnly { implicit s =>
          Sql.getJobs
            .map(JobDBAll(_))
            .list
            .apply()
          ...
          Sql
            ...
            .map { rs =>
              ...
              StepRun(
                ...
                res.properties,
                res.status,
                getTimestampAsString(res.inserted_at),
                res.elapsed_time,
                res.step_type,
                res.step_run_id
              )
