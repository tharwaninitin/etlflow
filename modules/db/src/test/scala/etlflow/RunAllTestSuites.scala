...
object RunAllTestSuites extends ZIOSpecDefault with DbSuiteHelper {
  ...
  def spec: Spec[TestEnvironment, Any] = (suite("DB Test Suites")(
    ...
    audit.db.DbTestSuite.spec, // Will execute actual DB queries
    audit.db.SqlTestSuite.spec
  ) @@ TestAspect.sequential).provideShared(db.liveDBWithLog(credentials, jri).orDie)
}
