...
object DbLayerTestSuites extends DefaultRunnableSpec  with DbSuiteHelper{
  ...
  override def spec: ZSpec[environment.TestEnvironment, Any] =
    (suite("DBApi Suite")(
      ...
      testM("executeQueryWithResponse Test")(
        assertM(DBApi.executeQueryWithResponse[getDb]("select job_name from job").foldM(ex => ZIO.fail(ex.getMessage), _ => ZIO.succeed("Done")))(equalTo("Done"))
      ),
      ...
      testM("executeQueryWithSingleResponse Test")(
        assertM(DBApi.executeQueryWithSingleResponse[getDb]("select job_name from job").foldM(ex => ZIO.fail(ex.getMessage), _ => ZIO.succeed("Done")))(equalTo("Done"))
      )
    ) @@ TestAspect.sequential).provideCustomLayer(liveDBWithTransactor(credentials).orDie)
}