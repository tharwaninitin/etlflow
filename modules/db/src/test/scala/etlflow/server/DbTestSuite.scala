...
object DbTestSuite {
  ...
  val jobDbAll = List(
    JobDBAll("Job1", "", "", 0, 0, true, None),
    JobDBAll("Job2", "", "", 0, 0, false, None),
    JobDBAll("Job3", "", "", 0, 0, true, None)
  )
  val stepRuns = List(
  ...
  val spec: ZSpec[environment.TestEnvironment with DBServerEnv, Any] =
    suite("DB(server) Suite")(
      ...
      testM("getStepRuns Test")(
        assertM(
          DBServerApi
            .getStepRuns(DbStepRunArgs("a27a7415-57b2-4b53-8f9b-5254e847a301"))
            .map(x => x.map(sr => sr.copy(start_time = "")).sortBy(_.job_run_id))
        )(equalTo(stepRuns.sortBy(_.job_run_id)))
      ),
      testM("getJobRuns Test")(
        assertM(
          DBServerApi
            .getJobRuns(DbJobRunArgs(None, None, None, None, None, 10L, 0L))
            .map(x => x.map(sr => sr.copy(start_time = "")).sortBy(_.job_name))
        )(equalTo(jobRuns.sortBy(_.job_name)))
      ),
      testM("getJobLogs Test")(
        assertM(DBServerApi.getJobLogs(JobLogsArgs(None, Some(10L))).map(x => x.sortBy(_.job_name)))(
          equalTo(jobLogs.sortBy(_.job_name))
        )
      ),
      ...
      testM("addCredential Test")(
        assertM(DBServerApi.addCredential(Credential("AWS1", "JDBC", "{}")).map(x => x))(
          equalTo(Credential("AWS1", "JDBC", "{}"))
        )
      ),
      testM("updateCredential Test")(
        assertM(DBServerApi.updateCredential(Credential("AWS1", "JDBC", "{}")).map(x => x))(
          equalTo(Credential("AWS1", "JDBC", "{}"))
        )
      ),
      testM("refreshJobs Test")(
        assertM(DBServerApi.refreshJobs(List(EtlJob("Job1", Map.empty), EtlJob("Job2", Map.empty))).map(x => x))(
          equalTo(List(JobDB("Job1", "", true), JobDB("Job2", "", false)))
        )
      )
