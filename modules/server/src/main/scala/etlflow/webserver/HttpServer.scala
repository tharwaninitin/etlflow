  ...
  def etlFlowWebServer(auth: Authentication, config: Option[WebServer]) : ServerTask[Nothing] =
    for {
      ...
      app1               = Http.collect[Request] {
        case _ -> !! / "about" => Response.text(s"Hello, Welcome to EtlFlow API ${BI.version}, Build with scala version ${BI.scalaVersion}")
        case _ -> !! => Response(data = HttpData.fromStream(ZStream.fromResource("static/index.html")))
        case _ -> !! / "assets" / "js" / "2.70d81952.chunk.js" => Response(data = HttpData.fromStream(ZStream.fromResource("static/assets/js/2.70d81952.chunk.js")))
        case _ -> !! / "assets" / "js" / "main.9b2263d7.chunk.js" => Response(data = HttpData.fromStream(ZStream.fromResource("static/assets/js/main.9b2263d7.chunk.js")))
        case _ -> !! / "assets" / "css" / "2.f4ede277.chunk.css" => Response(data = HttpData.fromStream(ZStream.fromResource("static/assets/css/2.f4ede277.chunk.css")))
        case _ -> !! / "assets" / "css" / "main.2470ea74.chunk.css" => Response(data = HttpData.fromStream(ZStream.fromResource("static/assets/css/main.2470ea74.chunk.css")))
      }
