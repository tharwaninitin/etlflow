...
case class ZioAuthentication(cache: CaffeineCache[String], config: Option[WebServer]) extends  ApplicationLogger {
  ...
  def middleware[R, E](app: HttpApp[R, E]): HttpApp[R, E] =  Http.flatten {
    Http.fromFunction[Request](req => {
      val headerValue = if(req.getHeader(CaseInsensitiveString("Authorization")).isDefined)
        req.getHeader(CaseInsensitiveString("Authorization"))
      else
        req.getHeader(CaseInsensitiveString("X-Auth-Token"))
      headerValue match {
        case Some(value) =>
