...
import etlflow.api.Schema.{EtlJob, GQLEnv}
...
trait ServerSuiteHelper extends DbManager with EtlFlowUtils {
  ...
  val testAPILayer: ZLayer[Blocking, Throwable, GQLEnv] = ApiImplementation.live[MEJP](cache,testJobsSemaphore,List.empty,testJobsQueue,config,etlJob_name_package)
