    ...
    suite("Rest Scheduler Suite")(
      testM("Test scheduler with Job1")(
        for {
          ...
          fiber    <- etlFlowScheduler(testCronJobs,jobs).provideCustomLayer(testAPILayer ++ testDBLayer).fork
          _        <- TestClock.adjust(ZDuration.fromScala(Duration(3,MINUTES)))
          _        <- fiber.interrupt
        } yield assertCompletes
      ),
      testM("Test scheduler with no jobs")(
        etlFlowScheduler(testCronJobs, List.empty).provideCustomLayer(testAPILayer ++ testDBLayer).as(assertCompletes)
      )
