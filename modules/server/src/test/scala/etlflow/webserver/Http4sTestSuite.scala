...
object Http4sTestSuite extends DefaultRunnableSpec with Http4sServer with ServerSuiteHelper {
  ...
  val corsConfig  = GetCorsConfig(config.webserver)

  private def apiResponse(apiRequest: Request[ServerTask]): ServerTask[Either[String, String]] = allRoutes[MEJP](auth,corsConfig).use { routes =>
    for {
