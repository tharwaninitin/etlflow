...
abstract class HttpRunnableSpec(port: Int) {
  ...
  def statusPost(path: Path, headers: Headers = Headers.empty): ZIO[EventLoopGroup with ChannelFactory, Throwable, Status] =
  ...
  def requestPathPost(path: Path, headers: Headers): ZIO[EventLoopGroup with ChannelFactory, Throwable, Client.ClientResponse] =
  ...
  def request(path: Path, method: Method, content: String): ZIO[EventLoopGroup with ChannelFactory, Throwable,  Client.ClientResponse] = {
