...
object RestTestSuite extends DefaultRunnableSpec with ServerSuiteHelper with Http4sDsl[ServerTask] {

  zio.Runtime.default.unsafeRun(runDbMigration(credentials,clean = true))
  val env = (testAPILayer ++ testDBLayer).orDie

  private def apiResponse(apiRequest: Request[ServerTask]): ServerTask[Either[String, String]] =
    (for {
      client <- Task(Client.fromHttpApp[ServerTask](RestAPINew.routes.orNotFound))
  ...
  override def spec: ZSpec[environment.TestEnvironment, Any] =
    (suite("Rest Test Suite")(
      testM("Test REST runjob end point with correct job name") {
        val apiRequest: Request[ServerTask] = Request[ServerTask](method = POST, uri = uri"/runjob/Job1")
      ...
      testM("Test REST runjob end point with incorrect job name") {
        val apiRequest: Request[ServerTask] = Request[ServerTask](method = POST, uri = uri"/runjob/InvalidJob")
