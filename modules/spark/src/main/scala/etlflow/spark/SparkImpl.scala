...
object SparkImpl {
  val live: ZLayer[SparkSession, Throwable, SparkApi] = ZLayer.fromFunction { spark: SparkSession =>
    new Service {
      override def LoadDSHelper[T <: Product : TypeTag](location: Seq[String], input_type: IOType, where_clause: String): Task[Map[String, String]] = Task(ReadApi.LoadDSHelper[T](location,input_type,where_clause))
